...
async function readCookiesFromFile(filePath) {
  try {
    ...
    console.log('Parsed cookies:', cookies); // 调试输出，查看解析结果
    return cookies;
  } catch (error) {
    console.error('Error reading cookies from file:', error);
    return [];
  }
}
...
async function detectErrorImage(page) {
  return new Promise((resolve) => {
    // 创建一个 MutationObserver 实例
    page.evaluate(() => {
      return new Promise((observerResolve) => {
    ...
    }).then(resolve); // 在 Promise 解析后回调 resolve
  });
}
